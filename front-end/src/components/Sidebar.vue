  ...
  <section :class="collapsed? 'sidebar closed': 'sidebar open'" id="sidebar">
      <div class="container">
        ...
        <div class="default_handles child">
            <button class="handle"><uil-estate class="icon"></uil-estate> Home</button>
            <button class="handle"><uil-search class="icon"></uil-search> Search</button>
            <button class="handle"><uil-star class="icon"></uil-star> Favorites</button>
        </div>
        <div class="handles child">
            <p class="subtitle">collections</p>
            <div  v-for="topic in topics" class="topic" :key="topic.name">
              <button class="handle" :id="topic.name" @click="toggleAccordion(topic.name)">{{topic.name}} <span class="drop_down icon"><uil-angle-down></uil-angle-down></span></button>
              <div class="children collapsed">
                <button v-for="child in topic.content" class="handle" :key="child">{{child}}</button>
              </div>
            </div>
        </div>
...
<script>
import { UilEstate, UilSearch, UilStar, UilAngleLeft, UilAngleDown } from '@iconscout/vue-unicons';
...
export default {
  ...
  methods: {
    ...
    toggleAccordion(id){
      ...
      let children = toggle.nextElementSibling;
